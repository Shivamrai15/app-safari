...
interface UseQueueProps {
    ...
    reorderQueue: (reorderedUpNext: (Song & { album: Album })[]) => void;
}
...
export const useQueue = create<UseQueueProps>((set, get) => ({
    ...
    reorderQueue: (reorderedUpNext: (Song & { album: Album })[]) => {
        const currentSong = get().queue[0];
        if (currentSong) {
            set({ queue: [currentSong, ...reorderedUpNext] });
        } else {
            set({ queue: reorderedUpNext });
        }
    }
})
);